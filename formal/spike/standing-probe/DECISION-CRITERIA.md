...
[Author note: written 2026-08-30]

In this section I've written down what I think Tessera is doing based
upon my own goals for Tessera.  One of the challenges in working with
AI for a complex project like this is that drift happens and is quite
real, so I will explain what I think it should be doing because I
think that's what has been agreed.

1. What evidence is verified?
Tessera is an attestation service.  Like a traditional notary, it takes documents provided to it by an unaffiliated third party and verifies that the document was presented to them. Unlike a notary, Tessera is based upon the use of _cryptographic_ signatures, thus Tessera only needs to be able to verify that the public signing key is one that appears to be bound to the signature on the document.  Thus, the notary checks identification documents, but Tessera merely confirms that there is a valid cryptographic signature over a document.  The attestation applies to a bundle of documents as well - a contract signed by both parties that also refers to separate artifacts (documents, git repositories, or other data objects that themselves have some sort of cryptographic signature, such as a cryptographic hash.)  Unlike a notary, Tessera doesn't review the documents, it reviews the cryptographic signature over the document.  Thus a SHA-512 hash, which is considered to be "quantum-resistant" or eXtended Merkle Signature Scheme (XMSS), which was designed to be "quantum resistant" can be presented to Tessera as part of the attested package.  Thus, Tessera takes a bundle of documents, encodes metadata about how the attestation is generated, along with a cryptographically generated timestamp and uses its own private key to sign the entire bundle.  This verifies that a set of documents (identified by their own cryptographic signatures) were presented to Tessera and Tessera generated a signature over that package, plus Tessera's own metadata, and returned the signed package, in a published format (versioned, so it can be changed if necessary, which does not invalidate prior attestations) to the party requesting attestation.

2. Who could forge or equivocate about it?
The core idea is that forging is difficult.  A cryptographic hash of sufficient complexity is known to be computationally intractable to compute.  SHA-2 and SHA-3 cryptographic hashes, for example, are used in blockchain protocols (including bitcoin and Ethereum) precisely because they are tamper resistant.  Real risks aren't in faking the signatures it is in losing the keys.  An attestation service given a document signed by a compromised key has no guarantee that the document isn't forged.  A service could add an additional service to verify a public key is not revoked, but that is not a fundamental part of the service.  The strength of the system is to anchor specific artifacts to a specific timestamp, which makes "back-dating" difficult to do.  A self-attestation is, broadly speaking, without strong probative value.  The base signatures for Tessera, for example, are themselves self-attested, but tied to externally visible mechanisms; this is the bootstrap problem.  For Tessera, the real concern is similar: a key can be compromised.  When a key is compromised, the service can write a tombstone.  The original approach to this, which I am unclear if it has survived is that the attestation includes a signature of a value that will be included in the next attestation (the forward link,) as well as a tie to the prior attestation (e.g., the hash of the prior attestation. The specific technique is fixed by the implementation but must create a generation chain.)  The function of this is to make key compromise visible - two different attestations using the same forward key reference cannot tell us which attestation was authorized, but it can tell us the key itself is compromised.  The usual approach to mitigating risk is to bound the key lifetime.  When a key is compromised, the existing chain is terminated (the tombstone) and a new key must be created and independently attested, unlike when the old key is used to attest the replacement key.  Note: by "key" here I mean "public key" - the private key should never be visible and best practice is to maintain such keys inside a trusted execution environment.  Equivocation (repudiation) is outside the scope of Tessera, except for its own keys.  Third party keys are subject to their own revocation - Tessera does not ordinarily claim that the key(s) used by third parties have not been compromised: this is the responsibility of the parties at the time adversarial review of the attested package is being adjudicated.

3. What does an artifact presented alone establish?
First, let me note that my "work case" is one in which I ask: "what does an adjudicator need when presented with an attestation that is old to verify that the attestation is valid?"  One of the key constraints here is that Tessera insists this _does not include any dependency on any Tessera service surviving_.  That is why the format is documented, so a third party can review the package 80 years from now and connect it with the other artifacts.  Note that Tessera won't own those artifacts, it will only own a cryptographic value (or set of values) that were computed from the artifact(s).  I also note that an artifact can in fact itself consist of other Tessera artifacts.  For example, as cryptographic technology continues to evolve and new threats emerge after post-quantum cryptography becomes the system under attack, a new attestation can be created that incorporates all the prior attestations of the original bundle, further tied back to the block chain.  There is no "revise in place", there is only "these prior versions of the same bundle are now further re-attested in the current time."  The reason for this is that revision would weaken the system, not strengthen it, because even if SHA-256 is compromised in some fashion, the bitcoin blockchain (for example) that we use with our own OTS timestamps has numerous references to it, so rewriting it leaves prints that indicate the compromise.  Tessera doesn't claim to provide absolute tamper resistance, its claim is that it makes forging or backdating prior acts difficult.  The decision on whether or not an attestation survives in the future is the responsibility of a decision maker at that time: Tessera just seeks to provide them with enough information to make a decision that is justifiable given the preserved evidence.

4. What remains unproved?
The biggest unproved hole is that the model itself is correct.  While we have made every effort to ensure that we have formal proofs of the structural guarantees and the temporal invariants, we have not proven that our model is verifiably correct, nor have we proven that our implementation of our model maps to the model that we have constructed and proven.  We accept that it is distinctly possible that, despite our best efforts given the resources available to us, that there is no flaw in what we have constructed that we are unable to so.  Thus, we build Tessera knowing that review and verification is an ongoing part of the Tessera service, within resource constraints, so long as Tessera continues to operate.  To that end, we have built Tessera as an open source project.  Our software engineering discipline uses the same techniques, with OTS timestamps and SHA-1 (git) hashes, in a chain.  Note that the choice of SHA-1 is not one we would make today, but it is outside our immediate control; it should be sufficient to a high degree (but not absolute) certainty. Tessera has been designed to address potential future concerns, but we have no privileged information, so it is distinctly possible that there may be technological changes that we have not foreseen that might make Tessera malfunction.

Note: I did not review the RESULTS-PROBE.md file, despite it living in the same directory as this document. [2026-08-30: Tony Mason, Author]
