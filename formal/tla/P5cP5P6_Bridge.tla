...
(*                                                                          *)
(* Ship's guard is the full three-conjunct temporal predicate (the P5     *)
(* temporal test inside VALID_STRICT) on chain quantities — not           *)
(* conjunct 3 alone. Design-time finding (registered in  *)
...
(*   The retry loop is P5c's discharged territory. CORRECTION            *)
(*   (2026-07-21, round-2 non-author review): the original claim here     *)
(*   that the retry loop is "orthogonal to the ship/verify seam" is       *)
(*   REFUTED by this module's own WallClockDivergence witness — Ship is   *)
(*   legal at wall time past declared + Delta while P5c's fused-clock     *)
(*   expiry would have discarded/refused the attempt, so the two models   *)
(*   admit different outcomes away from the boundary. Which clock         *)
(*   expires an attempt is a PROTOCOL RULING (routed to the author);      *)
(*   this module's Ship gains a lifecycle guard, or its absence gets      *)
(*   registered, per that ruling. An unshipped anchored attempt here      *)
(*   doubles as the abandoned-anchor artifact of A2.0.                     *)
(*   RULING (2026-07-21, clock-precedence): separate clock roles — chain  *)
(*   time governs evidence, wall time governs the lifecycle with          *)
(*   operational slack S; no hard backward-lag bound registered; the      *)
(*   opportunity guarantee is conditional on observed lag B - C <= S.     *)
(*   Per the round-2 disposition, the ABSENCE of a lifecycle guard in     *)
(*   this module's Ship is hereby REGISTERED: this module checks the      *)
(*   chain-evidence seam only; the lifecycle seam is checked in           *)
(*   P5cP5P6_BridgeSlack{,_Latch}.tla. Ruling archive:                    *)
(*   docs/reviews/2026-07-21-clock-precedence-ruling.md.                   *)
