...
(*                                                                          *)
(* CLOCK-ROLES RULING (2026-07-21, resolves the round-2 A2 blocker):      *)
(* chain time governs evidence; wall time governs the attempt lifecycle   *)
(* and refusal. Under the ruling this module's single clock READS AS THE  *)
(* WALL CLOCK: its expiry `now > declared + Delta` is the S = 0 instance  *)
(* of the ruled lifecycle envelope `declared + Delta + S`; the slack      *)
(* analysis lives in P5cP5P6_BridgeSlack{,_Latch}.tla, and eligibility    *)
(* latching (a timely-eligible attempt cannot be expired by scheduling    *)
(* delay) is a construction obligation registered with the ruling, not    *)
(* modeled here. The coincidence noted above — Ship's fused guard IS the  *)
(* chain predicate — is a property of the single-clock abstraction only;  *)
(* decoupled, the seams are checked in the bridge and its companions.     *)
...
(* durability, not retrievability, not reporting. Those three are         *)
(* implementation/handoff obligations routed to Amendment 3, not          *)
(* silently absorbed here — and A2.3's draft text now carries the same    *)
(* split itself (aligned 2026-07-21; its earlier "reported" wording was   *)
(* narrowed on review).                                                    *)
