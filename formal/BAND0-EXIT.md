...
| E5 | Capstone composition context with discharge matrix | A3 §A3.3 (verbatim gate); ENUM §3; TRK cross-cutting box 2 | **partial** — **E5a done as a draft**: `formal/suite/LEDGER.md` (PROPOSED, fifth draft 2026-09-15) enters the 34 offered entries under A7 §A7.10; four Codex full reviews and one skeptic read applied (`docs/reviews/2026-09-14-codex-review-ledger-and-k6.md`, `2026-09-15-codex-full-review-ledger*.md`), all findings accepted; 29 scratch severing runs archived in `formal/suite/ledger-tests-2026-09-14/`. **Author RULED 2026-09-15, outcome 5** (R-1 = Reading B, composed; R-2 = yes; R-3 = formulation W; `LEDGER.md` §5): the cross-model matrix is **D1–D11, eleven rows**, §2(b) vacated, D12/D13 carried as §2(c) rows; §6 is the capstone contract. **E5b in progress**: `formal/suite/capstone/PREDICTIONS.md` (PROPOSED, drafted 2026-09-15 from §6 under the ruling), skeptic-read; **fourteen** owner-run Codex full passes (2026-09-16 to 2026-09-17; records `docs/reviews/2026-09-1{6,7}-codex-review-capstone-predictions*.md`; thirty-four findings accepted and repaired over rounds 1–11; **round 12 found no new run-backed defect, re-ran all 87 archived diagnostics with every result matching, and judged the file ready to freeze subject to the author's items**; a full skeptic read the same evening (`docs/reviews/2026-09-17-skeptic-read-capstone-predictions.md`, seven mechanical repairs) , round 13 confirming them with three one-sentence corrections, and **round 14 confirming those: ready to freeze subject to the author's items**, each round's findings landing on the previous round's repair; the composition contract stable since round 4), about a hundred scratch reproductions (`cap1`–`cap26`, README batches five to seventeen); one routed fork (the `REFUSAL` tag) and four contestable clerk choices for the author at the freeze (same-key wrapping; compromised-channel enrolment; `ev` as the pair in strict; the report contract under companions). `capstone.pv` absent; nothing run; **author RULED 2026-09-18 on the `REFUSAL` fork: option (i), scoped** — C-Q10 (the D-6 tag-separation judge) and C-C18 (the tag-severing companion) registered by dated amendment **before** the freeze, batch eighteen `cap27a`–`cap27f` (27 entries, 1185 min per case, EIGHTEEN companions); a full skeptic read and Codex round 16 on the amended version | **the author's one read**, routed 2026-09-17 (`formal/suite/capstone/READ-FREEZE-2026-09-17.md`): the **freeze commit**, the `REFUSAL` fork, the four contestable choices: the **freeze commit** containing `PREDICTIONS.md`, the `REFUSAL` fork answered (**done 2026-09-18**, option (i) scoped), the four contestable choices confirmed (**done 2026-09-18**, all four confirmed; the path veto accepted, the model identified by its contents); then build, results, cross-family review, author read (E5c–f) |
...
| E18 | Confirmation-depth evidence at the registered parameter (Amendment 7 §A7.8, author ruling 2026-09-14): either (a) P5c, P5P6 and bridge re-run at `DepthK = 5` with bounds sized so every registered witness fires, or (b) a non-author-reviewed argument that every checked invariant is independent of `DepthK` above the witnesses' minimum | A7 §A7.8; `formal/tla/READING-AID-P5c.md:226-231` | **done, path (a), 2026-09-14** — `formal/tla/k6-2026-09-14/RESULTS-K6.md`: P5c green with all seven witnesses and both companions as registered at `Delta = 6, MaxTime = 26, DepthK = 5`; bridge green on four invariants over 86.8M states at `KConf = 6, DepthK = 5, MaxBlocks = 7, MaxTime = 9, MaxSkew = 1`, all six witnesses fire, three companions red/green as registered; P5P6 has no depth constant. Two instance facts recorded on the reading aid (`Delta ≥ DepthK`; skew ±1 needed for closure at seven blocks) | non-author review: Codex partial read 2026-09-14 confirmed the bridge search closed and is "not yet my E18 sign-off" (`docs/reviews/2026-09-14-codex-review-ledger-and-k6.md`); full pass owed. Path (b) not needed |
...
2026-09-14: the eleven §12 items were ruled; instrument Amendment 7, signed at `9863513`. Nothing blocks today. Two items will be routed when ready: the P8 form decision (E3a, with criteria first) and the E9 sitting. The §5 readings stand unless vetoed; the E6 tracker rows are added as PROPOSED record facts in the same tree change as this file.
