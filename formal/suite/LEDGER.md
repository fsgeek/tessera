...
than silently omitting them. *(AUTHOR RULING 2026-09-18, capstone `PREDICTIONS.md` §8 item 1, option (i) scoped: the capstone exercises `REFUSAL` **as a signing domain** with an honest foreign-tag signer and registers C-Q10 and C-C18; `REFUSAL` stays unexercised **as a record** (L2-n, E6 untouched). This sentence's purpose — not silently omitted — stands; its wording is superseded for `REFUSAL` by the ruling.)* The seven-argument `framed` constructor is **NOT promoted**:
