...
| 108 | `let framed(ot, =alg, =id, =fp(kX), mh, cv, pl) = fb` | the signed bytes name this algorithm, identity and fingerprint | `=fp(kX)`: **LOAD-BEARING** (F7). `=alg`, `=id`: **CARRIED** (F7: alg → A3 §1; id → F8). `ot`, `cv`: **BOUND, not checked by S-P3** — read into the pattern and never compared; validation obligations are P7's (object type) and P8/H1a's (canonVer) [corrected 2026-09-14 per the author's ruling, Amendment 7 §A7.10; previously "`ot`, `cv`: **CARRIED** (… object type → P7; canonVer → P8/H1a)"] |
